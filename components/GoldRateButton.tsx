...
export const GoldRateButton: React.FC = () => {
  ...
  const RATES = [
    { karat: '24K', label: t('goldRate.24K.label'), aed: 397 },
    { karat: '22K', label: t('goldRate.22K.label'), aed: 365 },
    { karat: '21K', label: t('goldRate.21K.label'), aed: 348 },
    { karat: '18K', label: t('goldRate.18K.label'), aed: 298 },
  ];
  ...
  return (
    <div
      style={{
        ...
        boxShadow: open ? '0 0 0 0 rgba(0, 0, 0, 0)' : `${isAr ? '1px' : '-1px'} 1px 3px rgba(0, 0, 0, 0.12)`,
      }}
      className={`fixed bottom-24 end-0 top-auto z-40 bg-[#144B3C] text-white max-w-[320px] md:top-1/2 md:bottom-auto md:-translate-y-[calc(50%-43px)] ${
        open
          ? 'w-[calc(100vw-32px)] h-[420px] rounded-s-md'
          : 'w-[28px] h-[96px] rounded-ss-[12px] rounded-es-[12px]'
      }`}
    >
      {!open && (
        <>
          {/* Top concave notch — radial-gradient curve biting into the top edge */}
          <div
            aria-hidden
            className="pointer-events-none absolute end-0"
            style={{
              top: `-${NOTCH - 1}px`,
              width: NOTCH,
              height: NOTCH,
              background: `radial-gradient(circle at ${isAr ? 'top right' : 'top left'}, transparent ${NOTCH}px, ${BRAND} ${NOTCH + 1}px)`,
            }}
          />
          ...
          <div
            ...
            style={{
              ...
              width: NOTCH,
              height: NOTCH,
              background: `radial-gradient(circle at ${isAr ? 'bottom right' : 'bottom left'}, transparent ${NOTCH}px, ${BRAND} ${NOTCH + 1}px)`,
            }}
          />
        </>
      )}
      ...
      <button
        ...
        aria-label={t('aria.openGoldRate')}
        style={{
      ...
      >
        <div className="flex items-start justify-between">
          ...
          <button
            ...
            aria-label={t('aria.closeGoldRate')}
        ...
        <p className="mt-3 text-[10px] tracking-[0.12em] uppercase text-white/60">
          {t('goldRate.disclaimer', { date: '03 Sep 2026' })}
        </p>
